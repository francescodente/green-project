<footer class="footer">
    <div class="container text-center mb-4">
        <div class="row">
            <div class="col-12 mt-4 mb-3">
                <button id="go-top-btn" class="btn round outline ripple text-center">
                    <i class="mdi dark mdi-chevron-up"></i>
                    <span class="text-sec-dark">Torna su</span>
                </button>
            </div>
            <div class="col-12 mt-3 mb-4">
                <a href="index.php" class="footer-logo d-block"><img src="images/logo/fruitracers_logo_small.png"></a>
            </div>
        </div>
        <div class="row">
            <div class="col-12 col-md-4 mb-3 mb-md-0 order-md-2">
                <h6 class="h-variant text-accent">Mappa del sito</h6>
                <ul>
                    <li><a href="index.php">Home</a></li>
                    <li><a href="index.php#about">Chi siamo</a></li>
                    <li><a href="index.php#contacts">Contatti</a></li>
                    <li><a href="companies.php">Partner</a></li>
                </ul>
            </div>
            <div class="col-12 col-md-4 mb-3 mb-md-0 order-md-3">
                <h6 class="h-variant text-accent">Social</h6>
                <div class="mb-3">
                    <a href="https://www.facebook.com" target="_blank" class="btn icon dark ripple" title="Facebook"><i class="mdi dark mdi-facebook-box"></i></a>
                    <a href="https://www.instagram.com" target="_blank" class="btn icon dark ripple" title="Instagram"><i class="mdi dark mdi-instagram"></i></a>
                </div>
                <h6 class="h-variant text-accent">Orari di consegna</h6>
                <ul class="timetable">
                    <li><span>LUN - VEN</span><span>8-13, 15-18</span></li>
                    <li><span>SAB, DOM</span><span>-</span></li>
                </ul>
            </div>
            <div class="col-12 col-md-4 order-md-1">
                <h6 class="h-variant text-accent">Privacy</h6>
                <a href="privacy-terms.php" class="d-block mb-3">Informativa</a>
                <h6 class="h-variant text-accent">Copyright</h6>
                <p class="m-0">&copy; <?php echo date("Y"); ?> Fruitracers™</p>
            </div>
        </div>
    </div>
</footer>
